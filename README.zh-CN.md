...
`codex-bridge` 现在的主要启动方式只有一种，就是位置参数启动。
实际只需要关心 3 种情况：

### 3 个参数

传入 `base_url`、`api_key`、`model`：
...
传入 `base_url`、`api_key`、`model`、`context_size`：
...
如果 `~/.codex-bridge-python/config.toml` 和
...
如果保存的配置不足以恢复启动参数，`codex-bridge` 会明确提示缺少哪些值。
